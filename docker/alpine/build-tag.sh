...
while getopts "hpb:n:o:s:i:r:z:" arg; do #after param has ":" need option
  case $arg in
...
COPY --from=builder /usr/src/myapp/docker/depend/biz/wait-for-it.sh /usr/src/maintain/wait-for-it.sh
...
# replace build docker-compose.yml
echo -e "# copy right
